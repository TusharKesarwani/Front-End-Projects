...
async function search(query) {
  ...
  for (let i = 1; i <= pokecount; i++) {
    const pokemonName = pokedex[i]["name"].toUpperCase();
    if (pokemonName.includes(query.toUpperCase())) {
      const listItem = document.createElement('li');
      listItem.textContent = i.toString() + '. ' + pokemonName;
      listItem.addEventListener('click', async function () {
        await getPokemon(i);
        updatePokemon.call({ id: i });
      });
