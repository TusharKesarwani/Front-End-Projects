...
function showHidePasscode(isHidden, val) {
  var passcode =
    val === 0
      ...
      : document.getElementById("inputPasswordConf");
  var showHide =
    val === 0
      ? document.getElementById("show-hide")
      : document.getElementById("show-hide-conf");
  if (isHidden) {
    passcode.type = "text";
...
function login() {
  ...
  if (validate(email, key)) {
    document.getElementById("error-box").innerHTML =
      '<i class="bi bi-check2-circle"></i> Login Success!';
    showMessage(1);
  } else {
    document.getElementById("error-box").innerHTML =
...
function signup() {
  ...
  if (validate(email, key)) {
    if (key === confirm) {
      document.getElementById("error-box").innerHTML =
        '<i class="bi bi-check2-circle"></i> Signup Success!';
      showMessage(1);
    } else {
      document.getElementById("error-box").innerHTML =
