function generate_maze(width, height) {
  ...
  while (nodes.length > 0) {
    ...
    world[curx][cury] = " ";
    if (cury > 2 && world[curx][cury - 2] == "#") directions += "u";
    if (curx > 2 && world[curx - 2][cury] == "#") directions += "l";
    if (curx < width - 3 && world[curx + 2][cury] == "#") directions += "r";
    if (cury < height - 3 && world[curx][cury + 2] == "#") directions += "d";

    if (directions.length == 0) {
      nodes.splice(curnode, 1); // remove this node from the list
      if (nodes.length == 0) break; // no nodes left, we are done
