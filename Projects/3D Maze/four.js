...
main = {
  ...
  start: function (event) {
    ...
    var skymats = [
      new THREE.MeshBasicMaterial({
        side: THREE.BackSide,
        map: THREE.ImageUtils.loadTexture("textures/skybox/left.jpg"),
      }), // posx
      new THREE.MeshBasicMaterial({
        side: THREE.BackSide,
        map: THREE.ImageUtils.loadTexture("textures/skybox/right.jpg"),
      }), // negx
      new THREE.MeshBasicMaterial({
        side: THREE.BackSide,
        map: THREE.ImageUtils.loadTexture("textures/skybox/up.jpg"),
      }), // posy
      new THREE.MeshBasicMaterial({
        side: THREE.BackSide,
        map: THREE.ImageUtils.loadTexture("textures/skybox/down.jpg"),
      }), // negy
      new THREE.MeshBasicMaterial({
        side: THREE.BackSide,
        map: THREE.ImageUtils.loadTexture("textures/skybox/front.jpg"),
      }), // posz
      new THREE.MeshBasicMaterial({
        side: THREE.BackSide,
        map: THREE.ImageUtils.loadTexture("textures/skybox/back.jpg"),
      }), // negz
    ];
  ...
  setup_input: function () {
    ...
    main.kibo.down("up", function () {
      main.scene_settings.camera_movement.up = true;
    });
    main.kibo.up("up", function () {
      main.scene_settings.camera_movement.up = false;
    });

    main.kibo.down("down", function () {
      main.scene_settings.camera_movement.down = true;
    });
    main.kibo.up("down", function () {
      main.scene_settings.camera_movement.down = false;
    });

    main.kibo.down("left", function () {
      main.scene_settings.camera_movement.left =
        main.scene_settings.camera_movement.rot_speed;
    });
    main.kibo.up("left", function () {
      main.scene_settings.camera_movement.left = 0;
    });

    main.kibo.down("right", function () {
      main.scene_settings.camera_movement.right =
        main.scene_settings.camera_movement.rot_speed;
    });
    main.kibo.up("right", function () {
      main.scene_settings.camera_movement.right = 0;
    });
  ...
  update: function () {
    ...
    if (
      main.scene_settings.camera_movement.up ||
      main.scene_settings.camera_movement.down
    ) {
      var speed = -main.scene_settings.camera_movement.speed; // assume forwards motion

      if (main.scene_settings.camera_movement.down) speed = -speed; // we want to go backwards, not forwards!

      var theta = main.camera.rotation.y,
        dx = speed * Math.sin(theta) * dt,
        dz = speed * Math.cos(theta) * dt;

      var xtile = Math.floor(
          (main.camera.position.x + dx) / main.scene_settings.tilesize + 0.5
        ),
        ztile = Math.floor(
          (main.camera.position.z + dz) / main.scene_settings.tilesize + 0.5
        );

      //document.getElementById("position").innerHTML = "(" + xtile + ", " + ztile + ")";

      if (
        typeof main.maze[xtile] == "undefined" ||
    ...
    if (main.scene_settings.camera_movement.jumping) {
      ...
      else cm.jump_speed_current -= cm.jump_speed_delta;
      ...
      if (cm.jump_current_height < 0) {
        // the jump has ended, reset everything ready for the next one
        cm.jumping = false;
