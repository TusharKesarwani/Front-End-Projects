(function () {
  var mobile = (function () {
    ...
    var tap = alloy.tap;
    var tapOrClick = PlatformDetection$1.detect().deviceType.isTouch()
      ? alloy.tap
      : click;
    ...
    var is = function (element, selector) {
      ...
      if (elem.nodeType !== ELEMENT$1) return false;
        ...
        function (key, okey, presence, prop) {
          ...
          return (function () {
            return presence.fold(
              ...
              function (fallbackThunk) {
                return optionDefaultedAccess(obj, key, fallbackThunk).bind(
                  bundleAsOption
                );
              },
              function (baseThunk) {
    ...
    var cleanClass = function (element) {
      var classList = supports(element)
    ...
    var toggle$1 = function (element, clazz) {
      return supports(element)
        ? element.dom().classList.toggle(clazz)
        : toggle(element, clazz);
    };
    ...
    var monitorEvent = function (eventName, initialTarget, f) {
      var logger =
        eventsMonitored === "*" || contains(eventsMonitored, eventName)
          ? (function () {
              var sequence = [];
              return {
    ...
    var orientation = function (onReceive) {
      return Receiving.config({
        channels: wrap$2(TinyChannels.orientationChanged(), {
          onReceive: onReceive,
        }),
      });
    };
    ...
    var internalSet = function (dom, property, value$$1) {
      ...
      if (isSupported(dom)) dom.style.setProperty(property, value$$1);
    };
    ...
    var getFirst = function (component, hConfig, hState) {
      return descendant$2(component.element(), "." + hConfig.itemClass()).bind(
        function (e) {
          return component.getSystem().getByDom(e).toOption();
        }
      );
    ...
    var getLast = function (component, hConfig, hState) {
      var items = descendants$1(component.element(), "." + hConfig.itemClass());
      var last$$1 =
        items.length > 0 ? Option.some(items[items.length - 1]) : Option.none();
    ...
    var inside = function (target) {
      return (
        (name(target) === "input" && get$1(target, "type") !== "radio") ||
    ...
    var focusIn = function (component, gridConfig, gridState) {
      descendant$2(component.element(), gridConfig.selector()).each(function (
        first
      ) {
    ...
    var focusIn$1 = function (component, flowConfig) {
      flowConfig
        ...
        .or(descendant$2(component.element(), flowConfig.selector()))
        .each(function (first) {
    ...
    var focusIn$3 = function (component, menuConfig) {
      descendant$2(component.element(), menuConfig.selector()).each(function (
        first
      ) {
          ...
          function (data) {
            internals[data.pname()] = single(
              ...
              function (detail, partSpec, partValidated) {
                return data
                  .factory()
                  .sketch(combine(detail, data, partSpec, partValidated));
              }
            );
          },
          function (data) {
    ...
    var composite = function (owner, schema, partTypes, factory, spec) {
      ...
      var detail = asStructOrDie$1(owner, schema, specWithUid, partSchemas, [
        partUidsSchema,
      ]);
    ...
    var findValueOfX = function (
      ...
      if (xValue < bounds.left) {
        ...
        var xOffset =
          Math.min(bounds.right, Math.max(xValue, bounds.left)) - bounds.left;
      ...
      return Slider.sketch({
        ...
        components: [
          ...
          Slider.parts().spectrum({
            ...
            behaviours: derive$2([
              Toggling.config({ toggleClass: Styles.resolve("thumb-active") }),
            ]),
          }),
          ...
          derive$2([
            detail.toggling().fold(Toggling.revoke, function (tConfig) {
              return Toggling.config(
                deepMerge({ aria: { mode: "checked" } }, tConfig)
              );
            }),
            Focusing.config({
      ...
      return deepMerge({
        ...
        behaviours: deepMerge(
          derive$2([
            ...
            Composing.config({ find: Option.some }),
            Keying.config(
              detail.movement().config()(detail, detail.movement())
            ),
          ]),
    ...
    var preserve$2 = function (f, container) {
      ...
      var refocus = active(ownerDoc).bind(function (focused) {
        ...
        return hasFocus$$1(container)
          ? Option.some(container)
          : descendant(container, hasFocus$$1);
      });
        ...
        return map(menus, function (spec, name) {
          var data = Menu.sketch(
            deepMerge(spec, {
              value: name,
              items: spec.items,
              markers: narrow$1(rawUiSpec.markers, ["item", "selectedItem"]),
      ...
      return has$1(el, transConfig.destinationAttr())
        ? Option.some({
            ...
            destination: constant(
              get$1(comp.element(), transConfig.destinationAttr())
            ),
          })
        : Option.none();
    };
    ...
    var getState = function (comp, transConfig, transState) {
      ...
      return has$1(e, transConfig.stateAttr())
        ? Option.some(get$1(e, transConfig.stateAttr()))
        : Option.none();
    };
    ...
    var events$9 = function (transConfig, transState) {
      return derive([
        run(transitionend(), function (component, simulatedEvent) {
          ...
          getCurrentRoute(component, transConfig, transState).each(function (
            route
          ) {
    ...
    var initEvents = function (editorApi, toolstrip, alloy) {
      ...
      var isRanged = function (sel) {
        return (
          !eq(sel.start(), sel.finish()) || sel.soffset() !== sel.foffset()
        );
      };
    ...
    var descendantRtl = function (scope, predicate) {
      var descend = function (element) {
        ...
        for (var i = children$$1.length - 1; i >= 0; i--) {
          ...
          if (predicate(child$$1)) return Option.some(child$$1);
    ...
    var factory$4 = function (detail, components$$1, spec, _externals) {
      ...
      return {
        ...
        behaviours: deepMerge(
          ...
          get$6(detail.toolbarBehaviours())
        ),
    ...
    var parts$3 = constant([
      group({
        ...
        overrides: function (detail) {
          return {
            domModification: { classes: [detail.markers().itemClass()] },
          };
        },
      }),
    ]);
    ...
    var isDangerous = function (event$$1) {
      ...
      return (
        ...
        !contains(["input", "textarea"], name(event$$1.target()))
      );
    };
    ...
    var classify = function (element) {
      ...
      var classifier =
        get$1(element, yScrollingData) === "true"
          ? classifyScrolling
          : classifyFixed;
    ...
    var scrollIntoView = function (cWin, socket, dropup, top, bottom) {
      ...
      if (top > greenzone || bottom > greenzone) {
        ...
        IosScrolling.moveOnlyScroll(socket, socket.dom().scrollTop + top).get(
          refreshCursor
        );
      }
    };
            ...
            editor: {
              ...
              onTapContent: function (evt) {
                ...
                if (name(target) === "img") {
                  ...
                  var component = realm
                    .system()
                    .getByDom(Element$$1.fromDom(editor.editorContainer));
