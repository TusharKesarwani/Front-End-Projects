  ...
  CKEDITOR.plugins.add("autolink", {
    ...
    init: function (c) {
      function f(a) {
        ...
        a = a.link.match(CKEDITOR.config.autolink_urlRegex)
          ? h.output(a)
          : k.output(a);
        if (c.plugins.link) {
      ...
      function l(a, b) {
        ...
        return (c = c[c.length - 1]) && e(c)
          ? { start: a.lastIndexOf(c), end: b }
          : null;
      }
      function e(a) {
        return (
          a.match(CKEDITOR.config.autolink_urlRegex) ||
          a.match(CKEDITOR.config.autolink_emailRegex)
        );
      }
      var h = new CKEDITOR.template(
          '\x3ca href\x3d"{link}"\x3e{text}\x3c/a\x3e'
        ),
        k = new CKEDITOR.template(
          '\x3ca href\x3d"mailto:{link}"\x3e{text}\x3c/a\x3e'
        );
      c.on("paste", function (a) {
        if (
          a.data.dataTransfer.getTransferType(c) !=
          CKEDITOR.DATA_TRANSFER_INTERNAL
        ) {
          var b = a.data.dataValue;
          -1 < b.indexOf("\x3c") ||
            !e(b) ||
            ((a.data.dataValue = f(b)), (a.data.type = "html"));
        }
      });
      if (!CKEDITOR.env.ie || CKEDITOR.env.edge)
        c.on("contentDom", function () {
          var a =
            c.config.autolink_commitKeystrokes ||
            CKEDITOR.config.autolink_commitKeystrokes;
          c.on("key", function (b) {
            if (-1 != CKEDITOR.tools.indexOf(a, b.data.keyCode)) {
              var d = CKEDITOR.plugins.textMatch.match(
                c.getSelection().getRanges()[0],
                l
              );
              if (
                d &&
