(function (root, factory) {
  ...
  Rickshaw.Graph = function (args) {
    ...
    this.configure = function (args) {
      ...
      var renderer =
        args.renderer || (this.renderer && this.renderer.name) || "stack";
  ...
  Rickshaw.Graph.Axis.X = function (args) {
    ...
    this.render = function () {
      ...
      this.ticks =
        this.staticTicks || Math.floor(this.graph.width / this.pixelsPerTick);
      var berth = Math.floor((this.width * berthRate) / 2) || 0;
      var bar_offset =
        (this.graph.renderer.name == "bar" &&
          Math.ceil(
            (this.graph.width * 0.95) / this.graph.series[0].data.length / 2
          )) ||
        0;
      var transform;
  ...
  Rickshaw.Graph.Axis.Y = Rickshaw.Class.create({
    ...
    setSize: function (args) {
      ...
      this.width =
        args.width || elementWidth || this.graph.width * this.berthRate;
  ...
  Rickshaw.Graph.DragZoom = Rickshaw.Class.create({
    initialize: function (args) {
      ...
      this.minimumTimeSelection =
        args.minimumTimeSelection || defaults.minimumTimeSelection;
  ...
  Rickshaw.Graph.HoverDetail = Rickshaw.Class.create({
      ...
      this.graph.series.active().forEach(function (series) {
        ...
        for (var i = approximateIndex; i < data.length - 1; ) {
          ...
          if (data[i].x <= domainX && data[i + 1].x > domainX) {
            dataIndex =
              Math.abs(domainX - data[i].x) < Math.abs(domainX - data[i + 1].x)
                ? i
                : i + 1;
            break;
          }
        ...
        var point = {
          ...
          formattedYValue: yFormatter(
            series.scale ? series.scale.invert(value.y) : value.y
          ),
          series: series,
          value: value,
          distance: distance,
    ...
    render: function (args) {
      ...
      var actualY = series.scale
        ? series.scale.invert(point.value.y)
        : point.value.y;
  ...
  Rickshaw.Graph.RangeSlider.Preview = Rickshaw.Class.create({
      ...
      if ("width" in args || "height" in args) {
        ...
        this.previews.forEach(function (preview) {
          var height =
            ...
            this.config.frameTopThickness * 2;
    ...
    render: function () {
      ...
      this.svg = d3
        ...
        .data([null]);
      this.previewHeight =
        this.config.height - this.config.frameTopThickness * 2;
      this.previewWidth =
        this.config.width - this.config.frameHandleThickness * 2;
      ...
      this.currentFrame[0] =
        currentWindow[0] === undefined
          ? 0
          : Math.round(domainScale.invert(currentWindow[0]));
      if (this.currentFrame[0] < 0) this.currentFrame[0] = 0;
      this.currentFrame[1] =
        currentWindow[1] === undefined
          ? this.previewWidth
          : domainScale.invert(currentWindow[1]);
      if (
        this.currentFrame[1] - this.currentFrame[0] <
        self.config.minimumFrameWidth
      ) {
        this.currentFrame[1] =
          (this.currentFrame[0] || 0) + self.config.minimumFrameWidth;
      }
      this.svg
        .enter()
        .append("svg")
    ...
    _renderFrame: function () {
      ...
      path +=
        " H " + (this.currentFrame[1] + this.config.frameHandleThickness * 2);
      ...
      [0.4, 0.6].forEach(
        function (spacing) {
          path +=
            " M " +
            Math.round(
              this.currentFrame[0] + this.config.frameHandleThickness * spacing
            ) +
            " " +
          ...
          path +=
            ...
            Math.round(
              this.currentFrame[1] +
                this.config.frameHandleThickness * (1 + spacing)
            ) +
            " " +
            Math.round(this.config.height * 0.3);
          path += " V " + Math.round(this.config.height * 0.7);
    ...
    _registerMouseEvents: function () {
      ...
      function onMousedown() {
        ...
        d3.select(document).on(
          "mousemove.rickshaw_range_slider_preview",
          onMousemove
        );
        d3.select(document).on(
          "mouseup.rickshaw_range_slider_preview",
          onMouseup
        );
        d3.select(document).on(
          "touchmove.rickshaw_range_slider_preview",
          onMousemove
        );
        d3.select(document).on(
        ...
        d3.select(document).on(
          "touchcancel.rickshaw_range_slider_preview",
          onMouseup
        );
      }
      ...
      element
        .select("rect.right_handle")
        .on("mousedown", onMousedownRightHandle);
      element
        .select("rect.middle_handle")
        .on("mousedown", onMousedownMiddleHandle);
      element
        .select("rect.left_handle")
        .on("touchstart", onMousedownLeftHandle);
      element
        .select("rect.right_handle")
        .on("touchstart", onMousedownRightHandle);
      element
        .select("rect.middle_handle")
        .on("touchstart", onMousedownMiddleHandle);
    },
  ...
  Rickshaw.Graph.Renderer = Rickshaw.Class.create({
    ...
    domain: function (data) {
      var stackedData =
        data || this.graph.stackedData || this.graph.stackData();
  ...
  Rickshaw.Graph.Renderer.Line = Rickshaw.Class.create(
  ...
  Rickshaw.Graph.Renderer.Stack = Rickshaw.Class.create(
    Rickshaw.Graph.Renderer,
    {
      name: "stack",
      defaults: function ($super) {
  ...
  Rickshaw.Graph.Renderer.Bar = Rickshaw.Class.create(Rickshaw.Graph.Renderer, {
    ...
    domain: function ($super) {
      ...
      var frequentInterval = this._frequentInterval(
        this.graph.stackedData.slice(-1).shift()
      );
    ...
    barWidth: function (series) {
      ...
      var barWidth =
        this.graph.x.magnitude(frequentInterval.magnitude) * (1 - this.gapSize);
      return barWidth;
    },
    render: function (args) {
      args = args || {};
  ...
  Rickshaw.Graph.Renderer.Area = Rickshaw.Class.create(
    Rickshaw.Graph.Renderer,
    {
      name: "area",
      defaults: function ($super) {
      ...
      _styleSeries: function (series) {
        ...
        if (this.stroke) {
          d3.select(series.path)
            ...
            .attr(
              "stroke",
              series.stroke || d3.interpolateRgb(series.color, "black")(0.125)
            )
  ...
  Rickshaw.Graph.Renderer.Multi = Rickshaw.Class.create(
    Rickshaw.Graph.Renderer,
    {
      name: "multi",
      initialize: function ($super, args) {
  ...
  Rickshaw.Graph.Renderer.LinePlot = Rickshaw.Class.create(
    Rickshaw.Graph.Renderer,
    {
      name: "lineplot",
      defaults: function ($super) {
    ...
    initialize: function (data, palette, options) {
      ...
      this.timeBase =
        typeof options.timeBase === "undefined"
          ...
          : options.timeBase;
      var timeInterval =
        typeof options.timeInterval == "undefined" ? 1e3 : options.timeInterval;
