    ...
    if ("object" == typeof i.ranges) {
      for (l in i.ranges) {
        ...
        if (
          (this.dateLimit &&
            m &&
            o.clone().add(this.dateLimit).isAfter(m) &&
            (m = o.clone().add(this.dateLimit)),
  ...
  return (
    (a.prototype = {
      ...
      renderCalendar: function (a) {
        ...
        for (
          var k = t([f, m, D, 12, o, h]), y = ((g = 0), 0), v = 0;
          g < 42;
          g++, y++, k = t(k).add(24, "hour")
        )
          g > 0 && y % 7 == 0 && ((y = 0), v++),
            (i[v][y] = k.clone().hour(r).minute(o).second(h)),
            k.hour(12),
            this.minDate &&
              i[v][y].format("YYYY-MM-DD") ==
                this.minDate.format("YYYY-MM-DD") &&
              i[v][y].isBefore(this.minDate) &&
              "left" == a &&
              (i[v][y] = this.minDate.clone()),
            this.maxDate &&
              i[v][y].format("YYYY-MM-DD") ==
                this.maxDate.format("YYYY-MM-DD") &&
              i[v][y].isAfter(this.maxDate) &&
              "right" == a &&
              (i[v][y] = this.maxDate.clone());
        "left" == a
          ...
          for (
            ...
            (!S || B >= b.month()) && (!A || B <= C.month())
              ...
              : (_ +=
                  ...
                  "</option>");
          _ += "</select>";
          for (var E = '<select class="yearselect">', W = L; W <= I; W++)
            E +=
              '<option value="' +
              W +
              '"' +
        ...
        for (v = 0; v < 6; v++) {
          (Y += "<tr>"),
            this.showWeekNumbers
              ? (Y += '<td class="week">' + i[v][0].week() + "</td>")
              : this.showISOWeekNumbers &&
                (Y += '<td class="week">' + i[v][0].isoWeek() + "</td>");
          for (y = 0; y < 7; y++) {
            var N = [];
            i[v][y].isSame(new Date(), "day") && N.push("today"),
              ...
              this.minDate &&
                i[v][y].isBefore(this.minDate, "day") &&
                N.push("off", "disabled"),
