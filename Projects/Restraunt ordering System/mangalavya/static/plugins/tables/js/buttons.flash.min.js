(function (g) {
  ...
  function u(a) {
    n === o &&
      (n =
        -1 ===
        v
          .serializeToString(g.parseXML(p["xl/worksheets/sheet1.xml"]))
          .indexOf("xmlns:r"));
  ...
  h.hasFlash = function () {
    try {
      ...
      if (
        navigator.mimeTypes &&
        navigator.mimeTypes["application/x-shockwave-flash"] !== o &&
    ...
    x = function (a, b) {
      for (
        var d = b.newline
          ...
          k = b.header ? i(c.header) + d : "",
          m = b.footer && c.footer ? d + i(c.footer) : "",
          l = [],
          t = 0,
          A = c.body.length;
        t < A;
  ...
  j.ext.buttons.excelFlash = g.extend({}, s, {
    ...
    action: function (a, b, d, c) {
      var a = c._flash,
        ...
        d = {
          _rels: { ".rels": g.parseXML(p["_rels/.rels"]) },
          xl: {
            _rels: {
        ...
        j = function (a) {
          ...
          for (var b = 0, c = a.length; b < c; b++) {
            for (var d = b, j = ""; 0 <= d; )
              (j = String.fromCharCode((d % 26) + 65) + j),
                (d = Math.floor(d / 26) - 1);
            d = j + "" + i;
      ...
      c.footer &&
        b.footer &&
        (j(b.footer, e), g("row:last c", f).attr("s", "2"));
      j = l(f, "cols");
      ...
      for (n = b.header.length; m < n; m++)
        j.appendChild(
          l(f, "col", {
            attr: { min: m + 1, max: m + 1, width: y(b, m), customWidth: 1 },
          })
        );
