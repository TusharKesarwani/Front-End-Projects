  ...
  (n.prototype = {
    ...
    serialize: function () {
      ...
      return (
        (step = function (s, i) {
          var a = [];
          return (
            s.children(e.options.itemNodeName).each(function () {
              var s = t(this),
                o = t.extend({}, s.data()),
    ...
    dragStart: function (e) {
      ...
      for (n = 0; n < h.length; n++)
        (d = t(h[n]).parents(this.options.listNodeName).length) >
          this.dragDepth && (this.dragDepth = d);
    },
    dragStop: function (t) {
    ...
    dragMove: function (i) {
      var a,
        ...
        d = this.options,
        h = this.mouse;
      this.dragEl.css({ left: i.pageX - h.offsetX, top: i.pageY - h.offsetY }),
        (h.lastX = h.nowX),
        (h.lastY = h.nowY),
        (h.nowX = i.pageX),
        (h.nowY = i.pageY),
      ...
      h.dirAx !== r
        ? ((h.distAxX = 0), (h.distAxY = 0))
        : ((h.distAxX += Math.abs(h.distX)),
          0 !== h.dirX && h.dirX !== h.lastDirX && (h.distAxX = 0),
          (h.distAxY += Math.abs(h.distY)),
          0 !== h.dirY && h.dirY !== h.lastDirY && (h.distAxY = 0)),
        (h.dirAx = r),
        h.dirAx &&
          h.distAxX >= d.threshold &&
          ((h.distAxX = 0),
          (n = this.placeEl.prev(d.itemNodeName)),
          h.distX > 0 &&
            n.length &&
            !n.hasClass(d.collapsedClass) &&
            ((a = n.find(d.listNodeName).last()),
            this.placeEl.parents(d.listNodeName).length + this.dragDepth <=
              d.maxDepth &&
              (a.length
                ? (a = n.children(d.listNodeName).last()).append(this.placeEl)
                : ((a = t("<" + d.listNodeName + "/>").addClass(
                    d.listClass
                  )).append(this.placeEl),
                  n.append(a),
                  this.setParent(n)))),
          h.distX < 0 &&
            (this.placeEl.next(d.itemNodeName).length ||
              ((l = this.placeEl.parent()),
              this.placeEl.closest(d.itemNodeName).after(this.placeEl),
              l.children().length || this.unsetParent(l.parent()))));
      var p = !1;
      if (
        (o || (this.dragEl[0].style.visibility = "hidden"),
        (this.pointEl = t(
          s.elementFromPoint(
        ...
        o || (this.dragEl[0].style.visibility = "visible"),
        this.pointEl.hasClass(d.handleClass) &&
          (this.pointEl = this.pointEl.parent(d.itemNodeName)),
        this.pointEl.hasClass(d.emptyClass))
      )
        p = !0;
      else if (!this.pointEl.length || !this.pointEl.hasClass(d.itemClass))
        return;
      ...
      if (!h.dirAx || g || p) {
        ...
        if (
          this.dragDepth - 1 + this.pointEl.parents(d.listNodeName).length >
          d.maxDepth
        )
          return;
        ...
        (l = this.placeEl.parent()),
          p
            ? ((a = t(s.createElement(d.listNodeName)).addClass(
                d.listClass
              )).append(this.placeEl),
          ...
          l.children().length || this.unsetParent(l.parent()),
          this.dragRootEl.find(d.itemNodeName).length ||
            this.dragRootEl.append('<div class="' + d.emptyClass + '"/>'),
          g &&
            ((this.dragRootEl = c),
    ...
    (t.fn.nestable = function (e) {
      ...
      return (
        this.each(function () {
          ...
          i
            ? "string" == typeof e && "function" == typeof i[e] && (s = i[e]())
            : (t(this).data("nestable", new n(this, e)),
