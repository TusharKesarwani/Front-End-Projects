...
async function countsort(heightarray, al) {
  let k = heightarray[0];
  for (let i = 0; i < al; i++) k = Math.max(k, heightarray[i]);

  let count = new Array(k + 1);
  for (let i = 0; i <= k; i++) count[i] = 0;

  for (let i = 0; i < al; i++) count[heightarray[i]]++;
  ...
  for (let i = 0; i < al; i++) {
    ...
    document.getElementById(
      array_of_bars[i].id
    ).style.height = `${heightarray[i]}px`;
    document.getElementById(array_of_bars[i].id).style.borderLeft =
      "2px solid red";
  }
}
